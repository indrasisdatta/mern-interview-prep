...
Cross site scripting (XSS) : XSS is when an attacker can run their own JS code by tricking the website that it's their own code.
    - Reflected XSS e.g. opens new link ?search=<script>alert(1)</script>
        server-side issue - search is sent as API payload
    - Stored (Persistent) XSS e.g. comment input <script> submitted and saved in DB. When other people views profile, the script gets executed to get their info
    - DOM-based XSS e.g. JS takes untrusted input from URL param and writes to DOM without proper sanitization  innerHTML X use textContent 
        URL: site.com?query=<img src="" onerror="alert(document.cookies)" />
        const url = URLSearchParams(window.location.search)
        HTML: innerHTML = url.get('name)
...
CSRF : browser can't distinguish between a legitimate user action and a forged request. 
    ...
    - Attacker's website contains form action as http://mybank.com and document.submit() so the bank website uses logged in user's cookie
...
HSTS (HTTP Strict Transport Security)
 ...
 - if http url is entered, it does 301 redirect for the first time to https 
 - browser remembers that for next 1 year (set in max-age), use https URL
 // nginx 
 add_header Strict-Transport-Security "max-age=31536000; includeSubDomains; preload" always;

CSRF protection:
 - Use Samesite=Lax cookies 
 - Check Origin (more reliable as can't be changed) & Referrer headers
 - For SPA - double submit pattern or synchronizer token 
 - Reauthentication for critical operations 

Synchronizer token example: 
 - During initial page load, API call is made to get CSRF token (stored in session)
 - Client side stores the csrf in redux and sends it as X-CSRF-TOKEN request header
...
Regular Expression DoS (ReDOS) attack - exploits the complexity of Regex operations to cause denial of service.  
...
Here is why running frontend builds in Docker minimizes risk:
1) Isolation (The Sandbox):
 - The app runs in its own container, isolated from your host system. 
 - If a malicious package tries to scan your SSH keys, it sees a blank wall.
2) Ephemeral environments:
 - Containers are short-lived. 
 - A compromise doesn't persist
 - Kill the container, you kill the threat
3) Minimal attack surface: 
 - Use node:alpine or slim images
 - Fewer packages means fewer holes to exploit
4) Locked-down installs: 
 - Stop malicious lifecycle scripts in their tracks:
