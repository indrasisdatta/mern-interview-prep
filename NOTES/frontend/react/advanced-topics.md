...
1. React advanced hooks 
   ...
   useDeferredValue, useTransition - controls UI rendering priority (interruptible by React - can pause, resume, discard renders)
      E.g. typing feels instant while UI updates are deferred 

   Debounce/throttle - control API call frequency, resize/scroll handlers 
      
   useTransition (State Control): You use this when you have access to the state-setting function (like setResult). You are essentially telling React: "Hey, when I call this function, don't rush the UI update."

   useDeferredValue (Value Control): You use this when you only have the value (usually passed down as a prop) and don't have control over the original setState call.
...
- When Provider’s value changes → All useContext() consumers re-render
